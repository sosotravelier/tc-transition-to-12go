...
## 6. Implementation Workflow Frameworks (April 2026 Evaluation)

### Superpowers (131K stars) — ADOPTED

Pure markdown skills for Claude Code with no runtime dependencies. Cherry-picked for implementation phase.

**What it provides**: TDD enforcement (RED-GREEN-REFACTOR), design-first brainstorming per task, systematic debugging (4-phase root cause investigation), verification-before-completion (evidence before claims), parallel subagent dispatch via git worktrees, subagent-driven development with two-stage review (spec compliance + code quality).

**Why adopted**: Purely additive to our existing setup. Provides *behavioral patterns* (how to approach a task) while our custom system provides *knowledge patterns* (what to know about the project). No runtime deps, no conflicts with existing skills. Largest community means ongoing maintenance.

**What we cherry-picked**: TDD, brainstorming, debugging, verification, parallel agents, git worktrees, writing-plans, executing-plans, subagent-driven-development, finishing-a-development-branch.

**Ref**: [GitHub](https://github.com/obra/superpowers)

### GSD v2 (4.1K stars) — SKIPPED (borrowed 1 principle)

TypeScript harness that solves "context rot" with fresh subagent contexts per task. Wave-based parallel execution with dependency tracking, crash recovery, cost tracking.

**Why skipped**: Requires separate TS process alongside Claude Code. Wave-based decomposition over-engineered for sequential endpoint porting. Conflicts with our existing orchestration skills.

**What we borrowed**: "A task must fit in one context window. If it cannot, it's two tasks." — added as CLAUDE.md line.

**Ref**: [GitHub](https://github.com/gsd-build/gsd-2)

### Claude Forge (626 stars) — SKIPPED

"oh-my-zsh for Claude Code" with 11 agents, 36 commands, 6-layer security hooks.

**Why skipped**: 36 commands collide with our existing 8+ skills. Weak community traction. Opinionated about entire lifecycle.

### Claude Code Toolkit (53 stars) — SKIPPED (borrowed 1 pattern)

Production-tested explore/plan/next/ship workflow with session handoff at 70% context usage.

**Why skipped**: Good patterns but not infrastructure. The 70% handoff is a CLAUDE.md line, not a framework.

**What we borrowed**: "At 70% context usage, summarize progress and start a fresh session."

### Key Industry Consensus (2026)

- Fresh context per task + persistent artifacts between tasks is the winning pattern
- Verification (tests/lint) is the bottleneck, not code generation
- CLAUDE.md should stay under ~200 lines; compliance drops beyond that
- Human-curated CLAUDE.md outperforms AI-generated by ~7%
- Spec quality is the primary lever — strong specs scale across agent fleets

---

## 7. Synthesis: What's Borrowable
